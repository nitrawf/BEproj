...
if __name__ == "__main__":
    ...
    X = np.array(df.loc[:,"Pregnancies":"Age"].values).T
    Y = np.array(df.loc[:,"Outcome"].values)
    ...
    Y_testset=Y[:,400:]
    n_h=7
    costs,parameters = nn_model(X_trainset, Y_trainset, n_h , num_iterations = 15000, print_cost=True)
    costs = np.squeeze(costs)
    with open("parameters.txt", "wb") as pf:
        pickle.dump({"parameters" : parameters, "mean" : Xmeans, "std" : Xdevs}, pf)
